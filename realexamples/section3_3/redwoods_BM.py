# when users apply this code to new datasets, they should carefully select suitable pertuarbation terms noise_var1 and noise_var2 added to covariance matrix and precision matrix.
#Convergence rate of latent GP relies on the choice of noise_var1 and noise_var2.
...
def expo_quad_kernel(xn,xm): # 1,0.1
    return min(xn[0],xm[0])*min(xn[1],xm[1])

def expo_quad_kernel2(xn,T1l,T1U,T2l,T2U): # 1,0.1
    return (T1U*xn[0]-0.5*(xn[0]**2)-0.5*(T1l**2))*(T2U*xn[1]-0.5*(xn[1]**2)-0.5*(T2l**2))

def expo_quad_kernel3(T1l,T1U,T2l,T2U): # 1,0.1
    return ((T1U**3)/3+2*(T1l**3)/3-(T1l**2)*T1U/2-(T1l**2)*T1U/2)*((T2U**3)/3+2*(T2l**3)/3-(T2l**2)*T2U/2-(T2l**2)*T2U/2)
...
points_inhomo=1000+np.concatenate((np.array(loc1).reshape(len(loc1),1),np.array(loc2).reshape(len(loc2),1)),axis=1)
...
noise_var1=1e-6 #adding to C
noise_var2=1e-5 #adding to \tilda{Q}
theta=np.exp(4)
...
for i in range(Nfinal):
    for j in range(i,Nfinal):
        ...
        if (j==(Nfinal-1)) and (i<(Nfinal-1)):
            cov_K[i][j]=expo_quad_kernel2(x4[i],T1l,T1U,T2l,T2U)
        if (j==(Nfinal-1)) and (i==(Nfinal-1)):
            cov_K[i][j]=expo_quad_kernel3(T1l,T1U,T2l,T2U)  
        if j!=i:
            cov_K[j][i]=cov_K[i][j]           

cov_K_inv=np.linalg.solve(cov_K+np.eye(Nfinal)*noise_var1,np.eye(Nfinal))

print(cov_K@cov_K_inv)
ones_vec=np.ones(Nfinal)
ones_vec[Nfinal-1]=(T1U-T1l)*(T2U-T2l)
...
min_eig=np.min(np.real(np.linalg.eigvals(cov_K_mod_inv)))
print(min_eig)
a=list(np.diag(cov_K_mod_inv))
ind=a.index(min(a))
cov_K_mod_inv_add=cov_K_mod_inv
cov_K_mod_inv_add[ind,ind]=cov_K_mod_inv_add[ind,ind]+noise_var2
cov_K_mod=np.linalg.inv(cov_K_mod_inv_add)
print(cov_K_mod@cov_K_mod_inv)
...
for ite in range(nsim1):
    ...
    beta_pos=beta+1/2*g_mk[0]@cov_K_inv@g_mk[0]
...
plt.figure()
plt.plot(np.array(g_mk_list2)[:,200])
plt.title('latent GP at midpoint',y=-0.2)
plt.savefig("/output/graphs/real3/BM_traceplot1.pdf",bbox_inches='tight')
plt.show()
...
plt.savefig("/output/graphs/real3/BM_histogram.pdf",bbox_inches='tight')
plt.show()
...
pdf_pos_new2=med.reshape(nn1,nn2).T
...
plt.title('Redwoods data with RI-BM fit',y=-0.2)
plt.legend()
plt.savefig("/output/graphs/real3/BM_median.pdf",bbox_inches='tight')

np.savez('/output/graphs/real3/real3_BM.npz', aaa=g_mk_list3,aa=g_mk_list2,a=g_mk_list,c=points_inhomo,s=theta_list)
